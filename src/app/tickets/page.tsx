...
export default function TicketsPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('tickets-cols');
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem('tickets-cols', JSON.stringify(visibleCols)); } catch { /* ignore */ }
  ...
  void allUpdates; void savedDetailsRef;
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <>
                ...
                {/* Autosave status */}
                <div style={{ height: '16px', textAlign: 'right' }}>
                  {saveStatus === 'saving' && <span style={{ fontSize: '11px', color: '#6b7d8f' }}>Saving…</span>}
                  {saveStatus === 'saved'  && <span style={{ fontSize: '11px', color: '#22cc6e' }}>Saved ✓</span>}
                </div>
